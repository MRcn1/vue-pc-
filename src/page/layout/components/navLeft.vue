<template>
  <div class='navLeft'>
    <el-col :span="3.5">
      <el-menu default-active="0" class="el-menu-vertical-demo" :default-openeds="openeds" active-text-color='#4c9dea'
        :unique-opened='true'>
        <el-submenu :index="num+''" v-for="(item,num) in classify" :key="num">
          ...
          <el-menu-item-group>
            <el-menu-item :index="num+'-'+ind" v-for="(value,ind) in newchildren" :key="ind"
              :class="value.path==$route.path?'active':''" @click="toPath(value.path)" v-if="item==value.meta.top">
